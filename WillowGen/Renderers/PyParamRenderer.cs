...
public class PyParamRenderer(TypedParamDef elem) : IRenderable
{
    public void Render(Sink sink)
    {
        ...
        else if (elem.IsFunctionParam)
        {
            ...
            {
                sink.Append($"{elem.Name()}: OptOut[{RendererUtils.GetTypeName(elem.ParamType)}]");
            }
            else if (elem.IsOptionalParam)
            {
                sink.Append($"{elem.Name()}: Opt[{RendererUtils.GetTypeName(elem.ParamType)}]");
            }
            else if (elem.IsOutParam)
            {
                sink.Append($"{elem.Name()}: Out[{RendererUtils.GetTypeName(elem.ParamType)}]");
            }
            else
            {
                sink.Append($"{elem.Name()}: {RendererUtils.GetTypeName(elem.ParamType)}");
